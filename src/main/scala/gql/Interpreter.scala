...
object Interpreter {
  ...
  def rePlan2[F[_]: Statistics: Monad](rootSel: NonEmptyList[PreparedField[F, Any]]): F[ExecutionDeps[F]] =
    Optimizer.costTree[F](rootSel).map(Optimizer.plan(_)).map { plan =>
      planExecutionDeps[F](rootSel, plan)
    }

  def rePlan[F[_]: Statistics: Monad](rootSel: NonEmptyList[PreparedField[F, Any]]) =
    Optimizer.costTree[F](rootSel).map(Optimizer.plan(_))

  def planAndRun[F[_]: Async: Statistics](rootInput: Any, rootSel: NonEmptyList[PreparedField[F, Any]]) =
    rePlan(rootSel).flatMap(run[F](rootInput, rootSel, _))
  ...
  )(implicit F: Async[F], stats: Statistics[F]): F[NonEmptyList[JsonObject]] =
    Supervisor[F].use { sup =>
      val flat = Optimizer.flattenNodeTree(plan)
