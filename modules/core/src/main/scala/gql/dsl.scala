...
//   def lift[I] = new PartiallyAppliedLift[I]
...
//   // final case class PartiallyAppliedUnionVariant[A, B](name: String) extends AnyVal {
//   //   def apply[F[_]](pf: PartialFunction[A, B])(implicit innerTpe: => Type[F, B]) =
//   //     PartiallyAppliedUnion1[F, A](name, Variant[F, A, B](Eval.later(innerTpe))(pf.lift))
//   // }

//   // final case class PartiallyAppliedUnionSubtype[A, B](name: String) extends AnyVal {
//   //   def apply[F[_]](implicit ev: B <:< A, ct: ClassTag[B], innerTpe: => Type[F, B]) = {
//   //     val pf: PartialFunction[A, B] = { case a: B => a }
//   //     PartiallyAppliedUnion1[F, A](name, Variant[F, A, B](Eval.later(innerTpe))(pf.lift))
//   //   }
//   // }

//   final case class PartiallyAppliedUnion0[F[_], A](name: String) extends AnyVal {
//     // def variant[B] = new PartiallyAppliedUnionVariant[A, B](name)
//     def variant[B](pf: PartialFunction[A, B])(implicit innerTpe: => Type[F, B]): PartiallyAppliedUnion1[F, A] =
//       PartiallyAppliedUnion1[F, A](name, Variant[F, A, B](Eval.later(innerTpe))(pf.lift))

//     // def subtype[B] = new PartiallyAppliedUnionSubtype[A, B](name)
