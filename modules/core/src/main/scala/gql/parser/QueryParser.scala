...
object QueryParser {
  ...
  sealed trait Value
  sealed trait ConstantValue extends Value
  object Value {
    final case class VariableValue(v: String) extends Value
    final case class IntValue(v: BigInt) extends ConstantValue
    final case class FloatValue(v: BigDecimal) extends ConstantValue
    final case class StringValue(v: String) extends ConstantValue
    final case class BooleanValue(v: Boolean) extends ConstantValue
    case object NullValue extends ConstantValue
    final case class EnumValue(v: String) extends ConstantValue
    final case class ListValue(v: List[Value]) extends ConstantValue
    final case class ObjectValue(v: List[(String, Value)]) extends ConstantValue
  }
